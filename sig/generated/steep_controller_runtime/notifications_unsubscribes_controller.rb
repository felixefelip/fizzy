...
class Notifications::UnsubscribesController
  def render(target = nil, *rest)
    @__rbs_infer__performed = true
    case target
    when :new then ERBNotificationsUnsubscribesNew.new(user: @user).__rbs_infer__body
    when :show then ERBNotificationsUnsubscribesShow.new(user: @user).__rbs_infer__body
    end
    true
  end
