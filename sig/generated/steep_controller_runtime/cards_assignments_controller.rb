...
class Cards::AssignmentsController
  def render(target = nil, *rest)
    ...
    case target
    when :create then ERBCardsAssignmentsCreate.new(card: @card).__rbs_infer__body
    when :new then ERBCardsAssignmentsNew.new(assigned_to: @assigned_to, card: @card, users: @users).__rbs_infer__body
    end
    true
  end
