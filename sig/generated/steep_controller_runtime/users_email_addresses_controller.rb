...
class Users::EmailAddressesController
  def render(target = nil, *rest)
    @__rbs_infer__performed = true
    case target
    when :create then ERBUsersEmailAddressesCreate.new(user: @user).__rbs_infer__body
    when :new then ERBUsersEmailAddressesNew.new(page_title: @page_title, user: @user).__rbs_infer__body
    end
    true
  end
